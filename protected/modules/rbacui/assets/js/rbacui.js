    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error == 'none') {
                ...
                $('#' + authItemTypeName[data.type] + '-info').load(url2, {'item': data.name}, function () {
                    $('#' + authItemTypeName[data.type] + '-info').toggleClass('spinner')
                });
            } else {
                settings = $('#attach-item-form').data('settings');
...
function selectAuthItem(obj, url) {
    ...
    $('#' + obj.id + '-info').html('');
    $('#' + obj.id + '-info').load(url, request, function () {
        $('#' + obj.id + '-info').toggleClass('spinner')
    });
...
function selectUser(url) {
    ...
    if (user.length == 1) {
        ...
        $('#user-info').html('');
        $('#user-info').load(url, {user: user, name: name}, function () {
            $('#user-info').toggleClass('spinner')
        });
    } else {
...
function tabLoaded(obj, url1, url2) {
    if (obj.id == 'tab1') {
        if ($("#tempuser").text() != "") {
            ...
            $('#user-info').toggleClass('spinner');
            $('#user-info').load(url2, {user: user, name: name}, function () {
                $('#user-info').toggleClass('spinner')
            });
            $('input[type=button]').attr('disabled', false);
        }
        $("#search").autocomplete({
            ...
            select: function (event, ui) {
                ...
                $('#user-info').toggleClass('spinner');
                $('#user-info').html('');
                $('#user-info').load(url2, {user: ui.item.value, name: ui.item.label}, function () {
                    $('#user-info').toggleClass('spinner')
                });
                $('input[type=button]').attr('disabled', false);
                return false;
